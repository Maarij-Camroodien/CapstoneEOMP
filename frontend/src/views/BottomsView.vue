<template>
    <div>
        <div>
        <div>
         <h1><span style="color: black;">BOTTOMS</span></h1>
        </div>
        <div>
    ...
    <div class="collapse navbar-collapse" id="navbarNavAltMarkup">
      <div class="navbar-nav mx-auto">
        ...
        <div class="my-5 container" >   
            <div class="row" style="margin-top: 3rem;font-family: 'Merriweather', serif;" v-if="bottoms">
      <div class="car col-12 col-sm-6 col-md-4 p-2" v-for="product in bottoms" :key="product.prodID">
                  <img :src="product.prodUrl" :alt="product.prodName" style="width:14rem;height:18rem;" loading="lazy">
                  <div class="card-body">
                      <br>
                      <h5 class="card-title">{{ product.prodName }}</h5>
                      <h5 class="card-title">{{ product.Category }}</h5>
                      <h5 class="card-text">R {{ product.amount }}</h5>
                      <router-link :to=
                      "{name: 'single',
...
<script>
    export default {
        ...
        mounted() {
            this.$store.dispatch('fetchBottoms')
        }
